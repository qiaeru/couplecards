...
function pickTranslation(all, locale) {
  const chosen = all[locale]
    ? locale
    : all[FALLBACK_LOCALE]
      ? FALLBACK_LOCALE
      : SUPPORTED_LOCALES.find((l) => all[l]);
...
function deckVersion() {
  ...
  const rev =
    getDb().prepare(`SELECT value FROM settings WHERE key = 'deck_revision'`).get()?.value ?? '0';
...
export default async function cardRoutes(app) {
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      const updates = [];
      const args = [];
      if (request.body.pile !== undefined) {
        updates.push('pile = ?');
        args.push(request.body.pile);
      }
      if (request.body.foil !== undefined) {
        updates.push('foil = ?');
        args.push(request.body.foil ? 1 : 0);
      }
      if (request.body.emoji !== undefined) {
        updates.push('emoji = ?');
        args.push(request.body.emoji ?? null);
      }
      if (request.body.sortOrder !== undefined) {
        updates.push('sort_order = ?');
        args.push(request.body.sortOrder);
      }
