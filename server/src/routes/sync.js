...
export default async function syncRoutes(app) {
  ...
  app.get('/state', async (request) => {
    ...
    `,
      ...
      .all(userId)
      .map((r) => ({ cardId: r.card_id, bannedAt: r.banned_at }));
    const history = db
      .prepare(
        `
  ...
  app.delete(
    ...
    async (request) => {
      const db = getDb();
      db.prepare('DELETE FROM bans WHERE user_id = ? AND card_id = ?').run(
  ...
  app.delete(
    ...
    async (request) => {
      getDb()
        .prepare('DELETE FROM history WHERE user_id = ? AND client_uuid = ?')
