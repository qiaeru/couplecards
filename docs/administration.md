...
3. Select a pile, write a title and write a description. Enable the **foil** flag to mark the card as a rare variant: it gets a gold border and a "Rare" label on the card and is intentionally drawn less often than standard cards (see [architecture.md](./architecture.md) for the draw weighting).
