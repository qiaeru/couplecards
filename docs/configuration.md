...
| `COOKIE_SECURE` | `false` outside Docker, `true` in the HTTPS variants (set in each Compose file, not in `.env`) | When set to `true`, the session cookie carries the `Secure` flag. This must be `true` whenever the site is served over HTTPS, and `false` for any plain-HTTP deploy (`http://localhost`, LAN IP, NAS). Trap to remember: a stock Docker container runs with `NODE_ENV=production`, which defaults this variable to `true`. If you then expose the app over plain HTTP without explicitly setting `COOKIE_SECURE=false`, the browser silently drops every session cookie and login appears to "succeed" but loops back to the login screen. The server logs a warning the first time it sees a plain-HTTP request in this misconfigured state. |
| `TRUST_PROXY` | `false` outside Docker, `true` in the HTTPS variants (set in each Compose file, not in `.env`) | Tells Fastify to read `X-Forwarded-*` headers. Enable it only when the server sits behind a trusted reverse proxy you control. When the app is exposed directly to the internet, leaving this `false` is important: otherwise a remote caller can spoof `X-Forwarded-For` and the per-IP rate limits (login, change-password, global `/api/*`) become trivial to bypass. |
| `SEED_LOCALE` | `en` with the Compose files; outside Docker, `fr` when `LANG` is French, otherwise `en` | Starting interface language of the admin account, and of the demo account, created on first boot. Accepts `en`, `fr`, `de`, `it`, or `es`. Each account can change it later in its settings. The deck always ships in all five languages. |
