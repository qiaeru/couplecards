...
- Signing out wipes all three stores, so a device shared between accounts never falls back to the previous user's cached data when `GET /api/state` is unreachable. The last signed-in user is kept in the `state` store: when the server is out of reach at startup, the app opens as that user from the cached data, and when a different account signs in on the device, the stores are wiped before it loads.
